...
router.post('/updelete',function(req, res, next) {
    ...
    //发送查询语句
    connection.query("UPDATE `news` SET `newstime`=?,`newstitle`=?,`newsimg`=?,`newstype`=? WHERE `newsid`=?", [newstime, newstitle, newsimg, newstype, newsid], function(err, rows) {
        res.json(rows);
...
router.post('/insert',function(req, res, next) {
    ...
    var newstype = xss(req.body.newstype);
    //发送查询数据
    connection.query('INSERT INTO `news`( `newstitle`, `newstime`, `newsimg`, `newstype`) VALUES (?,?,?,?)', [newstitle, newstime, newsimg, newstype], function(err, result) {
        res.json("添加信息成功");
    });
});
